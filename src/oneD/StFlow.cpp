    ...
    m_tfixed(-1.)
{
    ...
    setBounds(c_offset_Tsolid, 200, 2*m_thermo->maxTemp()); // no bounds for inactive component
                          ...
                          double rdt, size_t jmin, size_t jmax)
{
    ...
    for (size_t j = jmin; j <= jmax; j++) {
        ...
        if (j == 0) {
            ...
            rsd[index(c_offset_Tsolid, 0)] = x[index(c_offset_Tsolid, j)];
        } else if (j == m_points - 1) {
            evalRightBoundary(x, rsd, diag, rdt);
            // set residual of poisson's equ to zero
            rsd[index(c_offset_E, j)] = x[index(c_offset_E, j)];
            rsd[index(c_offset_Tsolid, j)] = x[index(c_offset_Tsolid, j)];
